...
여러분들의 OS에 따라 설치하는 과정이 다르기 때문에, 제가 Window/Mac을 모두 커버하는 것은 무리가 있다고 판단했습니다. 그래서 생활코딩의 egoing 님의 동영상을 보고, 자신의 OS에 맞는 방법을 `Node.js`를 설치해주세요!!
...
`npm`은 여러 `Node.js`에서 사용할 수 있는 라이브러리를 손쉽게 설치해주는 도구입니다. `python`의 `pip`이나 `conda` 같은 녀석이라고 이해하면 될 것 같습니다.
...
역사적으로는 `JavaScript`가 1995년 웹브라우저에 처음 탑재되었고, 2009년 `JavaScript`를 윈도우/Mac과 같은 일반적인 환경에서도 작동할 수 있도록 만든 것이 `Node.js`입니다.
