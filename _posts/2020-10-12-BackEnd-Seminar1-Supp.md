...
## type 변환하기
JavaScript의 타입 변환에는 크게 두 가지가 있는데, 하나는 **암묵적 타입 변환**<small>implicit coercion</small>이고, 다른 하나는 **명시적 타입 변환**<small>explicit coercion</small>입니다. 둘의 뚜렷한 차이점은 `변수값의 재할당`에 있는데, 암묵적 타입 변환은 변수의 값을 재할당하는 것이 아니라 어떤 연산을 하기 위해 변환된 type의 값을 만들어 사용하고 변수 자체의 type은 변환되지 않습니다.
...
### 명시적 타입 변환
변환하고자 하는 변수를 x로 나타내어 예시를 들어보겠습니다!
